# target_app.py
# Versión protegida del archivo original: se basa en tu CEREBRITO_WEB_2025_v4.py
# - Antes de mostrar la UI verifica la sesión local y los créditos.
# - Si no hay token válido o no hay créditos, mostrará un mensaje indicando que se autentique.
import os
...
# Ruta del token de sesión local (compatibilidad con login.py)
...
def load_local_token():
    if SESSION_PATH.exists():
        try:
            ...
            return None
    # también buscar .session_token en carpeta local
    local = Path('.session_token')
    if local.exists():
        try:
...
def check_access():
    """Verifica token y créditos. Retorna (allowed:bool, reason:str, user)"""
    ...
    if not user.verified:
        return False, 'NOT_VERIFIED', user
    if (user.credits is None or user.credits <= 0) and not user.unlimited:
        return False, 'NO_CREDITS', user
    return True, 'OK', user

# Inicializar DB (si aún no se ha hecho)
init_db()

# Lógica original del archivo se incorpora debajo; para conservar tus funciones,
# incluimos el contenido original pero con la condición de acceso.
# Si el usuario no tiene acceso, mostramos una página Streamlit con instrucciones.

# A continuación insertamos el contenido original del archivo, pero lo encapsulamos
# en una función `run_app()` que solo se ejecuta si check_access permite.
...
# Nota: el archivo original no definía main(); definimos run_app para envolver la UI.
def run_app():
    # Ejecuta la porción principal de la app Streamlit del archivo original.
    # Si el archivo define objetos a nivel de módulo que ejecutan la UI al import,
    # es posible que ya se mostró. En ese caso, la protección debe residir en la parte superior.
    try:
        if 'st' in globals():
            # Cuando el archivo original ya construye la UI al importarlo, no hacemos nada adicional.
            pass
    except Exception:
        pass

if __name__ == '__main__':
    allowed, reason, user = check_access()
    import streamlit as st
    st.set_page_config(page_title='Cerebrito - Modo protegido', layout='wide')
    if not allowed:
        # Mostrar una página amable indicando qué hacer
        st.title('Acceso restringido — Autenticación requerida')
        if reason == 'NO_TOKEN':
            st.error('No se encontró token de sesión local. Usa el script `login.py` o inicia sesión en el panel de administración.')
        elif reason == 'INVALID_TOKEN':
            st.error('Token inválido o expirado. Vuelve a iniciar sesión con login.py o en el panel de administración.')
        elif reason == 'USER_NOT_FOUND':
            st.error('Usuario no encontrado. Contacta al administrador.')
        elif reason == 'NOT_VERIFIED':
            st.error('Usuario no verificado. Debes confirmar tu email antes de usar la aplicación.')
        elif reason == 'NO_CREDITS':
            st.error('No tienes créditos suficientes. Pide al administrador que te asigne créditos.')
        st.info('Accede al panel de administración con `streamlit run adminpanel.py` (necesitas ser admin) o usa el script login.py para usuarios verificados.')
    else:
        # Decrementar créditos si aplica y luego ejecutar la app
        db = SessionLocal()
        u = db.query(__import__('models').models.User).get(user.id)
        # decrementar
        if not u.unlimited:
            try:
                cost = int(os.getenv('CREDIT_COST_PER_RUN', '1'))
            except Exception:
                cost = 1
            if u.credits >= cost:
                ...
                st.error('No tienes suficientes créditos para ejecutar la aplicación.')
                db.close()
                raise SystemExit(1)
        db.close()
        # Ejecutar la UI importada (si el original ya dibujó la UI, esto puede ser redundante)
        run_app()
